...
## Decision-Making Process & Component Justification

To create this section a structured decision-making process was followed guided by project constraints, performance requirements, cost, and availability. Evaluating each category of components (microcontroller, power regulation, and sensors) by comparing options based on key criteria like functionality, efficiency, ease of integration, and compatibility. Selected final components that not only satisfied technical specifications but also offered reliability and practicality for prototyping and manufacturing.
...
The power budget was developed by identifying all critical system components, specifying their voltage requirements, and estimating their maximum current consumption. Each component on the +3.3V rail was analyzed for its contribution to total current demand.
